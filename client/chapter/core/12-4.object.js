...
const shopOrder = {
    ...
    totalPrice(){
        this.totla = this.menu.reduce((acc,cur) => {
...
let total = 0;
const result = shopOrder.menu.forEach((item,index,menu) => { return total += item.price * item.count});
console.log(total);

//reduce(acc,cur)
console.log(shopOrder.menu.reduce((acc,cur) => { return acc + (cur.price * cur.count) },0));
