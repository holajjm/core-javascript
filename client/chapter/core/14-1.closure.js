...
function first(){
    ...
    // return second(); // 함수를 실행한 값을 반환
    return second; // 함수 본문 자체를 반환
}
...
//다른 변수에 외부 함수를 할당해주면 새로운 실행 환경이 생겨나는 것이기 때문에 출력되는 값이 이어지지 않는다.

//함수는 태어난 환경을 기억합니다.따라서 외부 환경 참조가 가능한 것이다. 이것이 바로 클로저이다.
// 내장 함수를 외부 함수의 바깥에서 사용가능한 환경이 클로저 환경이다.
